...
export default function PositionalEncodingVisualizer() {
  ...
  const chartHeight = rowsCount * (40 + 32) - 32;
  ...
  const rows = useMemo(() => {
    ...
    for (const pair of selectedPairs) {
      ...
      const sinCenter = padTop + sinIndex * (40 + 32) + 40 / 2 + offset;
      ...
      const cosCenter = padTop + cosIndex * (40 + 32) + 40 / 2 + offset;
  ...
  const fontSize = containerWidth >= 1200 ? "18" : containerWidth >= 768 ? "16" : "14";
  const labelFontSize = containerWidth >= 1200 ? "18" : containerWidth >= 768 ? "16" : "12";
        ...
        <div className="h-full container mx-auto max-w-none flex justify-between items-center px-8">
          <div className="flex gap-5 items-center">
            ...
            <div className="space-y-2 flex-1">
              ...
              <p className="text-purple-300 leading-relaxed" style={{ fontSize: "20px", lineHeight: "1.3" }}>
            ...
            <div className="mt-10 p-3 rounded-lg bg-slate-800/50 border-l-4 border-l-purple-500 max-w-lg mx-auto">
              <p className="text-lg text-slate-300 leading-relaxed text-center">
                ...
                <span className="block">
                  <span className="font-semibold text-blue-300 text-base font-mono">Indices (i) {selectedPairs.join(", ")} </span>
                  <span className="ml-4 font-semibold text-pink-300 text-base font-mono">Position: {clampedHighlight} </span>
                </span>
              </p>
            </div>
          </CardContent>
        ...
        <Card className={`${cardBg} rounded-2xl shadow-2xl border flex-1 flex flex-col overflow-hidden min-h-0`}>
          <CardHeader className="pb-1 py-1">
            <CardTitle className="flex items-center gap-3 text-2xl px-2">
              ...
              <div className="flex-1 flex justify-center items-center gap-20 mx-4" style={{ transform: "translateX(-100px)" }}>
          ...
          <CardContent className="p-0 flex-1 flex min-h-0">
            <div ref={chartContainerRef} className="flex-1 overflow-x-auto overflow-y-auto min-h-0 w-full">
              ...
              >
                ...
                <text x={padLeft + chartWidth / 2} y={padTop + chartHeight + 70} textAnchor="middle" fill={textColor} fontSize={labelFontSize} fontWeight="600">
                  Token Position
                </text>
                ...
                <g>
                  <rect x={highlightX - 25} y={padTop + chartHeight + 20} width={50} height={20} fill="rgba(168, 85, 247, 0.9)" rx={4} stroke="rgba(255,255,255,0.3)" strokeWidth={1} />
                  <text x={highlightX} y={padTop + chartHeight + 33} textAnchor="middle" fill="white" fontSize="14" fontWeight="600" fontFamily="ui-monospace, monospace">
                    {clampedHighlight}
                  </text>
                </g>
                ...
                {rows.map((row, idx) => (
                  <g key={idx}>
                    ...
                    <text x={padLeft + chartWidth + 25} y={row.centerY + 2} fill={row.kind === "sin" ? waveSinEnd : waveCosEnd} fontSize={labelFontSize} style={{ fontFamily: "ui-monospace, monospace" }}>
                      {row.label}
                    </text>
                    <text x={padLeft + chartWidth + 25} y={row.centerY + 25} fill="#94a3b8" fontSize={labelFontSize} style={{ fontFamily: "ui-monospace, monospace" }}>
